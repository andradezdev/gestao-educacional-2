# ifitwala_ed/students/api/test_student_calendar.py
...
from ifitwala_ed.students.api.student_calendar import (
...
class TestStudentCalendar(TestCase):
    def test_get_student_calendar_includes_meetings(self):
        ...
        with (
            patch(
                "ifitwala_ed.students.api.student_calendar.frappe.session",
                frappe._dict({"user": "student@example.com"}),
            ),
            patch("ifitwala_ed.students.api.student_calendar.frappe.db.get_value", return_value="STU-0001"),
            patch("ifitwala_ed.students.api.student_calendar.frappe.cache", return_value=cache),
            patch("ifitwala_ed.students.api.student_calendar._system_tzinfo", return_value=tzinfo),
            patch(
                "ifitwala_ed.students.api.student_calendar._resolve_window",
                return_value=(window_start, window_end),
            ),
            patch("ifitwala_ed.students.api.student_calendar._get_student_enrolled_groups", return_value=[]),
            patch("ifitwala_ed.students.api.student_calendar._fetch_classes", return_value=[]),
            patch("ifitwala_ed.students.api.student_calendar._fetch_school_events", return_value=[]),
            patch(
                "ifitwala_ed.students.api.student_calendar._fetch_meetings",
                return_value=[meeting_event],
            ) as mocked_fetch_meetings,
            patch("ifitwala_ed.students.api.student_calendar.frappe.get_all", return_value=[]),
            patch(
                "ifitwala_ed.students.api.student_calendar.now_datetime",
                return_value=datetime(2026, 2, 1, 8, 0, 0),
            ),
        ):
    ...
    def test_invalidate_student_calendar_cache_removes_all_windows_for_resolved_student(self):
        ...
        with (
            patch("ifitwala_ed.students.api.student_calendar.frappe.cache", return_value=cache),
            patch(
                "ifitwala_ed.students.api.student_calendar.frappe.get_all",
    ...
    def test_refresh_student_calendar_views_invalidates_cache_and_publishes_realtime(self):
        ...
        with (
            ...
            patch("ifitwala_ed.students.api.student_calendar.frappe.publish_realtime") as mocked_publish,
        ):
            refresh_student_calendar_views(
